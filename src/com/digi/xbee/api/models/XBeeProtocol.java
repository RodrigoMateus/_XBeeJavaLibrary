...
/**
 * Enumerates the available XBee protocols. The XBee protocol is determined by
 * the combination of hardware and firmware of an XBee device.
 */
public enum XBeeProtocol {

	// Enumeration entries
	ZIGBEE(0, "ZigBee"), RAW_802_15_4(1, "802.15.4"), XBEE_WIFI(2, "Wi-Fi"), DIGI_MESH(3, "DigiMesh"), XCITE(4,
			"XCite"), XTEND(5, "XTend (Legacy)"), XTEND_DM(6, "XTend (DigiMesh)"), SMART_ENERGY(7,
					"Smart Energy"), DIGI_POINT(8, "Point-to-multipoint"), ZNET(9, "ZNet 2.5"), XC(10, "XSC"), XLR(11,
							"XLR"), XLR_DM(12, "XLR"), // TODO [XLR_DM] XLR
														// device with DigiMesh
														// support.
	UNKNOWN(99, "Unknown");
	 ...
	 * 
	 * @param id
	 *            The ID of the {@code XBeeProtocol} to retrieve.
	 * 
	 ...
	 *            The hardware version of the protocol to determine.
	 * @param firmwareVersion
	 *            The firmware version of the protocol to determine.
	 * 
	...
	public static XBeeProtocol determineProtocol(HardwareVersion hardwareVersion, String firmwareVersion) {
		if (hardwareVersion == null || firmwareVersion == null || hardwareVersion.getValue() < 0x09)
			...
			if ((firmwareVersion.length() == 4 && firmwareVersion.startsWith("8"))
				...
				// Edited by Rodrigo: to support Xtend radios on DigiMesh mode.
				// return XTEND_DM;
				return DIGI_MESH;
			return XTEND;
			...
			// future (after K64 integration) reporting the hardware and
			// firmware
			// version of the baseboard (see the case
			// HardwareVersionEnum.XLR_BASEBOARD).
			// TODO maybe this should be removed in future, since this case will
			// never be released.
			return XLR;
			...
			// and also firmware version (the one we have here is 1002, but this
			// value
			// is not being reported since is an old K60 version, the module fw
			// version
			// is reported instead).

			// TODO [XLR_DM] The next version of the XLR will add DigiMesh
